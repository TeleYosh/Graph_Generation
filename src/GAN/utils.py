    ...
    if dataset == 'test':
        ...
        if os.path.isfile(filename):
            ...
            for line in fr:
                ...
                desc_embedding = description_embeddings.get(graph_id, torch.zeros(768)).unsqueeze(0)
                data_lst.append(Data(stats=feats_stats, description_embedding=desc_embedding, filename = graph_id))
            fr.close()                    
        ...
        if os.path.isfile(filename):
            ...
            for fileread in tqdm(files):
                ...
                L = D - adj_bfs
                with np.errstate(divide="ignore"): # bedelt sp b np
                ...
                # Retrieve embedding for the description
                desc_embedding = description_embeddings.get(f"graph_{filen}", torch.zeros(768)).unsqueeze(0)

                data_lst.append(Data(x=x, edge_index=edge_index, A=adj, stats=feats_stats, description_embedding=desc_embedding, filename = filen))
            torch.save(data_lst, filename)
            print(f'Dataset {filename} saved')
    return data_lst
